...
====================
*/
static void ClipOccluders( idVec4 *verts, glIndex_t *indexes, int numIndexes, idVec3 projectionOrigin ) {
...
static void GenerateSilEdges( void ) {
	int				i, j;
	unsigned int	*edges = ( unsigned * )_alloca( ( numOutputTris * 3 + 1 ) * sizeof( *edges ) );
	int				numEdges = 0;
	numSilEdges = 0;
	for( i = 0 ; i < numOutputTris ; i++ ) {
...
void CleanupOptimizedShadowTris( srfTriangles_t *tri ) {
	...
	uniqued = new idVec3[maxUniqued];				// this is wrong you cannot use alloca on a class so use new
	numUniqued = 0;
	...
	glIndex_t *remap = new glIndex_t[maxUniqued];	// just for good measure we also changed this
