	...
	for( i = 0 ; i < numDrawSurfs ; i++ ) {
		...
		for( j = 0 ; j < tri->numIndexes ; j += 3 ) {
			...
			if( idMath::Fabs( area ) < 0.0001 ) {
				GL_Color( 0.0f, 0.0f, 1.0f, 0.5f );
			} else  if( area < 0 ) {
				GL_Color( 1.0f, 0.0f, 0.0f, 0.5f );
			} else {
				GL_Color( 0.0f, 1.0f, 0.0f, 0.5f );
			}
	...
	for( i = 0 ; i < numDrawSurfs ; i++ ) {
		...
		for( j = 0 ; j < tri->numIndexes ; j++ ) {
			...
			if( r_showTangentSpace.GetInteger() == 1 ) {
				GL_Color( 0.5f + 0.5f * v->tangents[0][0],  0.5f + 0.5f * v->tangents[0][1],
						  0.5f + 0.5f * v->tangents[0][2], 0.5f );
			} else if( r_showTangentSpace.GetInteger() == 2 ) {
				GL_Color( 0.5f + 0.5f * v->tangents[1][0],  0.5f + 0.5f * v->tangents[1][1],
						  0.5f + 0.5f * v->tangents[1][2], 0.5f );
			} else {
				GL_Color( 0.5f + 0.5f * v->normal[0],  0.5f + 0.5f * v->normal[1],
						  0.5f + 0.5f * v->normal[2], 0.5f );
			}
...
static void RB_ShowVertexColor( drawSurf_t **drawSurfs, int numDrawSurfs ) {
	...
	for( i = 0 ; i < numDrawSurfs ; i++ ) {
		...
		for( j = 0 ; j < tri->numIndexes ; j++ ) {
			const idDrawVert *v;
			v = &tri->verts[tri->indexes[j]];
			GL_Color( v->color[0], v->color[1], v->color[2], v->color[3] );
			glVertex3fv( v->xyz.ToFloatPtr() );
		}
		glEnd();
	}
