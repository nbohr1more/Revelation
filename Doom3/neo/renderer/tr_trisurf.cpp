...
void R_RangeCheckIndexes( const srfTriangles_t *tri ) {
	...
	if( tri->numVerts > tri->numIndexes ) {
		...
		common->DPrintf( "R_RangeCheckIndexes: tri->numVerts > tri->numIndexes\n" );
	}
}
...
void R_IdentifySilEdges( srfTriangles_t *tri, bool omitCoplanarEdges ) {
	...
	if( omitCoplanarEdges ) {
		...
		if( c_coplanarCulled ) {
			...
			common->DPrintf( "%i of %i sil edges coplanar culled\n", c_coplanarCulled, c_coplanarCulled + numSilEdges );
		}
	}
...
void R_TestDegenerateTextureSpace( srfTriangles_t *tri ) {
	...
	if( c_degenerate ) {
		common->DPrintf( "%d triangles with a degenerate texture space\n", c_degenerate );
	}
}
