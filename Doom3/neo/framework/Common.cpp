...
idCVar com_videoRam( "com_videoRam", "1024", CVAR_INTEGER | CVAR_SYSTEM | CVAR_NOCHEAT | CVAR_ARCHIVE, "holds the last amount of detected video ram" );
...
static void PrintMemInfo_f( const idCmdArgs &args ) {
	...
	f->Printf( "total(%s ) image(%s ) model(%s ) sound(%s ): %s\n",
			   ...
			   idStr::FormatNumber( mi.imageAssetsTotal ).c_str(),
			   idStr::FormatNumber( mi.modelAssetsTotal ).c_str(),
			   idStr::FormatNumber( mi.soundAssetsTotal ).c_str(), mi.filebase.c_str() );
	fileSystem->CloseFile( f );
}
