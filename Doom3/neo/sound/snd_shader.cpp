...
bool idSoundShader::CheckShakesAndOgg( void ) const {
	...
	for( i = 0; i < numLeadins; i++ ) {
		if( leadins[ i ]->objectInfo.wFormatTag == WAVE_FORMAT_TAG_OGG ) {
			common->DWarning( "sound shader '%s' has shakes and uses OGG file '%s'", GetName(), leadins[ i ]->name.c_str() );
			ret = true;
		}
	}
	for( i = 0; i < numEntries; i++ ) {
		if( entries[ i ]->objectInfo.wFormatTag == WAVE_FORMAT_TAG_OGG ) {
			common->DWarning( "sound shader '%s' has shakes and uses OGG file '%s'", GetName(), entries[ i ]->name.c_str() );
			ret = true;
		}
	}
	return ret;
...
void idSoundShader::List() const {
	...
	for( int k = 0; k < numLeadins ; k++ ) {
		...
		if( objectp ) {
			common->Printf( "      %5dms %4dKb %s (LEADIN)\n", soundSystemLocal.SamplesToMilliseconds( objectp->LengthIn44kHzSamples() ), ( objectp->objectMemSize / 1024 ), objectp->name.c_str() );
		}
	}
