...
ID_FORCE_INLINE float __fmuls( float a, float b )
{
	return ( a * b );
}
ID_FORCE_INLINE float __fmadds( float a, float b, float c )
{
	return ( a * b + c );
}
ID_FORCE_INLINE float __fnmsubs( float a, float b, float c )
{
	return ( c - a * b );
}
ID_FORCE_INLINE float __fsels( float a, float b, float c )
{
	return ( a >= 0.0f ) ? b : c;
}
ID_FORCE_INLINE float __frcps( float x )
{
	return ( 1.0f / x );
}
ID_FORCE_INLINE float __fdivs( float x, float y )
{
	return ( x / y );
}
ID_FORCE_INLINE float __frsqrts( float x )
{
	return ( 1.0f / sqrtf( x ) );
}
ID_FORCE_INLINE float __frcps16( float x )
{
	return ( 1.0f / x );
}
ID_FORCE_INLINE float __fdivs16( float x, float y )
{
	return ( x / y );
}
ID_FORCE_INLINE float __frsqrts16( float x )
{
	return ( 1.0f / sqrtf( x ) );
}
ID_FORCE_INLINE float __frndz( float x )
{
	return ( float )( ( int )( x ) );
}
...
ID_FORCE_INLINE __m128 _mm_msum3_ps( __m128 a, __m128 b ) {
	__m128 c = _mm_mul_ps( a, b );
	return _mm_add_ps( _mm_splat_ps( c, 0 ), _mm_add_ps( _mm_splat_ps( c, 1 ), _mm_splat_ps( c, 2 ) ) );
}

ID_FORCE_INLINE __m128 _mm_msum4_ps( __m128 a, __m128 b ) {
	__m128 c = _mm_mul_ps( a, b );
...
ID_FORCE_INLINE __m128 _mm_rcp32_ps( __m128 x ) {
...
ID_FORCE_INLINE __m128 _mm_rcp16_ps( __m128 x ) {
	__m128 r = _mm_rcp_ps( x );		// _mm_rcp_ps() has 12 bits of precision
	r = _mm_sub_ps( _mm_add_ps( r, r ), _mm_mul_ps( _mm_mul_ps( x, r ), r ) );
	return r;
}
...
ID_FORCE_INLINE __m128 _mm_div32_ps( __m128 x, __m128 y ) {
	return _mm_mul_ps( x, _mm_rcp32_ps( y ) );
}

// floating-point divide with at least 16 bits precision
ID_FORCE_INLINE __m128 _mm_div16_ps( __m128 x, __m128 y ) {
