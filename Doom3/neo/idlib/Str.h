...
ID_FORCE_INLINE const char *idStr::Left( int len, idStr &result ) const {
	return Mid( 0, len, result );
}

ID_FORCE_INLINE const char *idStr::Right( int len, idStr &result ) const {
	if ( len >= Length() ) {
		result = *this;
		return result;
	}
...
ID_FORCE_INLINE idStr idStr::Left( int len ) const {
	return Mid( 0, len );
}

ID_FORCE_INLINE idStr idStr::Right( int len ) const {
	if ( len >= Length() ) {
		return *this;
	}
