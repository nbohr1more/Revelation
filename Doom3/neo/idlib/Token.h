...
ID_FORCE_INLINE idToken::idToken( void ) {
}

ID_FORCE_INLINE idToken::idToken( const idToken *token ) {
	*this = *token;
}
...
ID_FORCE_INLINE void idToken::operator=( const char *text ) {
	*static_cast<idStr *>( this ) = text;
}

ID_FORCE_INLINE void idToken::operator=( const idStr &text ) {
	*static_cast<idStr *>( this ) = text;
}

ID_FORCE_INLINE double idToken::GetDoubleValue( void ) {
	if( type != TT_NUMBER ) {
		return 0.0;
	}
...
ID_FORCE_INLINE float idToken::GetFloatValue( void ) {
	return ( float ) GetDoubleValue();
}

ID_FORCE_INLINE unsigned long	idToken::GetUnsignedLongValue( void ) {
	if( type != TT_NUMBER ) {
		return 0;
	}
	if( !( subtype & TT_VALUESVALID ) ) {
...
ID_FORCE_INLINE int idToken::GetIntValue( void ) {
	return ( int ) GetUnsignedLongValue();
}

ID_FORCE_INLINE int idToken::WhiteSpaceBeforeToken( void ) const {
	return ( whiteSpaceEnd_p > whiteSpaceStart_p );
}

ID_FORCE_INLINE void idToken::AppendDirty( const char a ) {
