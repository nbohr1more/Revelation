...
template<class T> ID_FORCE_INLINE int	MaxIndex( T x, T y ) {
	return ( x > y ) ? 0 : 1;
}
template<class T> ID_FORCE_INLINE int	MinIndex( T x, T y ) {
	return ( x < y ) ? 0 : 1;
}

template<class T> ID_FORCE_INLINE T	Max3( T x, T y, T z ) {
	return ( x > y ) ? ( ( x > z ) ? x : z ) : ( ( y > z ) ? y : z );
}
template<class T> ID_FORCE_INLINE T	Min3( T x, T y, T z ) {
	return ( x < y ) ? ( ( x < z ) ? x : z ) : ( ( y < z ) ? y : z );
}
template<class T> ID_FORCE_INLINE int	Max3Index( T x, T y, T z ) {
	return ( x > y ) ? ( ( x > z ) ? 0 : 2 ) : ( ( y > z ) ? 1 : 2 );
}
template<class T> ID_FORCE_INLINE int	Min3Index( T x, T y, T z ) {
	return ( x < y ) ? ( ( x < z ) ? 0 : 2 ) : ( ( y < z ) ? 1 : 2 );
}

template<class T> ID_FORCE_INLINE T	Sign( T f ) {
	return ( f > 0 ) ? 1 : ( ( f < 0 ) ? -1 : 0 );
}
template<class T> ID_FORCE_INLINE T	Square( T x ) {
	return x * x;
}
