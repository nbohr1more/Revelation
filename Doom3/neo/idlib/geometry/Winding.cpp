...
void idWinding::AddToConvexHull( const idWinding *winding, const idVec3 &normal, const float epsilon ) {
	...
	newHullPoints = new idVec5[maxPts]; // V630
	hullDirs = new idVec3[maxPts];		// V630
	hullSide = new bool[maxPts];		// V630
...
void idWinding::AddToConvexHull( const idVec3 &point, const idVec3 &normal, const float epsilon ) {
	...
	hullDirs = new idVec3[numPoints];	// V630
	hullSide = new bool[numPoints];		// V630
	// calculate hull edge vectors
	for( j = 0; j < numPoints; j++ ) {
	...
	hullPoints = new idVec5[numPoints + 1]; // V630
	// insert the point here
	hullPoints[0] = point;
	numHullPoints = 1;
	// copy over all points that aren't double fronts
