...
void idInventory::RestoreInventory( idPlayer *owner, const idDict &dict ) {
	...
	} else if( g_itemMaxArmorType.GetInteger() == 2 ) { // V517
		...
		if( g_ammoUsageType.GetBool() ) {
			...
			if( weapon_classname ) {
				...
				if( decl ) {
					...
					if( g_ammoClipSizeType.GetInteger() == 1 && j >= 0 ) {  // V557
						max -= decl->dict.GetInt( "clipSize_doom" ) - clip[ j ];
					} else if( g_ammoClipSizeType.GetInteger() == 2 && j >= 0 ) {
...
void idPlayer::Think( void ) {
	...
	if( !g_stopTime.GetBool() ) {
		...
		if( !gameLocal.isMultiplayer ) {
			...
			if( g_useDynamicProtection.GetBool() && gameLocal.time - lastDmgTime > 500 ) {  // V571
				if( scale < 1.0f ) {
					scale += 0.05f;
				}
				if( scale > 1.0f ) {
